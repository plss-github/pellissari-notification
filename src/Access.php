...
final class Access
{
    ...
    {
        if (empty($_SESSION['techbell_csrf'])) {
            $_SESSION['techbell_csrf'] = bin2hex(random_bytes(32));
        }
        return $_SESSION['techbell_csrf'];
    }
    public static function post(Request $request): void
    {
        ...
        $provided = $request->request->get('_techbell_csrf');
        $expected = $_SESSION['techbell_csrf'] ?? '';
    ...
    public static function base(): string
    {
        ...
        return rtrim($CFG_GLPI['root_doc'] ?? '', '/') . '/plugins/techbell';
    }
}
