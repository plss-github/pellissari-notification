/* TechBell 0.1.1 | MIT | No third-party requests or dependencies. */
(() => {
  'use strict';
  if (window.__techbellLoaded) return;
  window.__techbellLoaded = true;
  const source = document.currentScript?.src || [...document.scripts].find(s => /\/techbell\/(?:public\/)?js\/techbell\.js/.test(s.src))?.src;
  if (!source) return;
  const match = new URL(source, location.href).pathname.match(/^(.*?)\/(?:plugins|marketplace)\/techbell\//);
  if (!match) return;
  const base = `${match[1]}/plugins/techbell`;
  const types = {
  ...
  function post(payload) {
    ...
    const work = async () => {
      ...
      return request('Action', {method: 'POST', body: new URLSearchParams({
        ...payload, _glpi_csrf_token: token.csrf, _techbell_csrf: token.plugin_csrf
      })});
    };
  ...
  function showNotice(message, error = false) {
    ...
    notice.classList.toggle('tb-error-text', error);
  }
  ...
  function build() {
    root = make('div', 'tb-root'); root.id = 'techbell-root';
    bell = button('', 'tb-bell', () => toggle());
    bell.innerHTML = icons.bell;
    bell.setAttribute('aria-label', 'Abrir central de notificações');
    bell.setAttribute('aria-controls', 'tb-panel'); bell.setAttribute('aria-expanded', 'false');
    bell.title = 'TechBell - Notificações';
    badge = make('span', 'tb-badge'); badge.hidden = true; bell.append(badge);
    panel = make('section', 'tb-panel'); panel.id = 'tb-panel'; panel.hidden = true;
    panel.setAttribute('role', 'dialog'); panel.setAttribute('aria-label', 'Central de notificações');
    const head = make('header', 'tb-panel-head');
    const heading = make('div'); heading.append(make('div', 'tb-eyebrow', 'TECHBELL'));
    ...
    const tools = make('div', 'tb-panel-tools');
    const tabs = make('div', 'tb-tabs');
    for (const [text, unread] of [['Todas', false], ['Não lidas', true]]) {
      const tab = button(text, `tb-tab${unread ? '' : ' is-active'}`, () => {
        state.unread = unread;
    ...
    refresh = button('Atualizar', 'tb-text-button', () => loadHistory(false));
    tools.append(tabs, refresh);
    const filter = make('select', 'tb-type-filter'); filter.setAttribute('aria-label', 'Filtrar por tipo de notificação');
    ...
    notice = make('p', 'tb-notice'); notice.hidden = true; notice.setAttribute('role', 'status');
    list = make('div', 'tb-notification-list');
    more = button('Carregar mais antigas', 'tb-btn tb-load-more', () => loadHistory(true)); more.hidden = true;
    const footer = make('footer', 'tb-panel-footer');
    readAll = button('Marcar todas como lidas', 'tb-text-button', async () => {
    ...
    configLink = make('a', 'tb-text-button', 'Configurar'); configLink.hidden = true;
    statusLine = make('div', 'tb-connection', 'Conectando...');
    footer.append(readAll, configLink);
    panel.append(head, tools, filter, notice, list, more, footer, statusLine);
    toastArea = make('div', 'tb-toast-area'); toastArea.setAttribute('aria-live', 'polite'); toastArea.setAttribute('aria-relevant', 'additions');
  ...
  function card(item, toast = false) {
    const node = make('article', `tb-notification${toast ? ' tb-toast' : ''}${item.read ? ' is-read' : ''}`);
    node.dataset.id = item.id;
    if (/^#[0-9a-f]{6}$/i.test(item.color)) node.style.setProperty('--tb-event-color', item.color);
    const line = make('div', 'tb-notification-top');
    const tag = make('span', 'tb-event-label', item.title); line.append(tag);
    if (toast) line.append(iconButton('close', 'Fechar aviso sem marcar como lido', () => node.remove()));
    else if (!item.read) line.append(make('span', 'tb-unread-dot'));
    const subject = make('strong', 'tb-ticket-title', `${item.ticket_id ? '#' + item.ticket_id + ' - ' : ''}${item.ticket_title}`);
    const message = make('p', 'tb-notification-message', item.message);
    const bottom = make('div', 'tb-notification-bottom');
    const time = make('time', '', niceDate(item.date)); time.dateTime = item.date; bottom.append(time);
    const actions = make('div', 'tb-notification-actions');
    ...
    if (item.url) actions.append(button('Abrir chamado', 'tb-open-ticket', () => openItem(item)));
  ...
  async function loadHistory(append) {
    ...
    if (!append) { state.cursor = null; list.replaceChildren(make('p', 'tb-empty', 'Carregando notificações...')); }
    try {
      ...
      if (!list.children.length) {
        const empty = make('div', 'tb-empty');
        const symbol = make('div', 'tb-empty-symbol'); symbol.innerHTML = icons.check;
  ...
  async function tick() {
    ...
    try {
      ...
      statusLine.classList.remove('tb-error-text');
      ...
      state.errors++; statusLine.textContent = e.message; statusLine.classList.add('tb-error-text');
  ...
  function bindAdmin() {
    const admin = document.querySelector('#tb-admin'); if (!admin) return;
    const feedback = admin.querySelector('#tb-admin-message');
    function message(text, error = false) { feedback.textContent = text; feedback.className = `tb-banner ${error ? 'tb-error' : 'tb-success'}`; }
    ...
    admin.querySelectorAll('.tb-test').forEach(test => test.addEventListener('click', async () => {
      test.disabled = true;
      try {
    ...
    const search = admin.querySelector('#tb-user-search');
    const results = admin.querySelector('#tb-user-results');
    const ruleBox = admin.querySelector('#tb-user-rules');
    const label = admin.querySelector('#tb-user-label');
    const save = admin.querySelector('#tb-user-save');
